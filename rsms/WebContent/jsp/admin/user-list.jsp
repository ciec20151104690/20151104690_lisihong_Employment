...
<body>
					...
					<form>
						<div class="form-group">
							<input type="text" class="form-control form-control-sm" id="name"
								placeholder="输入名称名称" value="${name}">
						</div>
						<div class="form-group">
							<input type="text" class="form-control form-control-sm" id="university"
								placeholder="输入学校名称" value="${university}">
						</div>
						<div class="form-group">
				...
				<table class="table">
					...
					<tbody>
						<c:if test="${userList.size() > 0}">
							...
							<tr onclick="showModel('${arr.id}')" class="<c:if test="${status.count % 2 == 0}">table-warning</c:if><c:if test="${status.count % 2 == 1}">table-info</c:if>">
								<td>${status.count}</td>
								...
								<td>
									<button onclick="delUser('${arr.id}')" type="button" class="btn btn-danger btn-sm">删除</button>
								</td>
							</tr>
							</c:forEach>
						</c:if>
	...
	<script type="text/javascript">
		...
		function search() {
			...
			window.location.href = "admin/user-list?name="+name+"&university="+university+"&curPage="+curPage;
		}
