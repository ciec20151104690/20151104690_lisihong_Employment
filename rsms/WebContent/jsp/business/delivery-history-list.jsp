...
<body>
				...
				<table class="table">
					...
					<tbody>
						<c:if test="${deliveryList.size() > 0}">
							...
							<tr onclick="showModel('${arr.user_id}')" class="<c:if test="${status.count % 2 == 0}">table-warning</c:if><c:if test="${status.count % 2 == 1}">table-info</c:if>">
								<td>${status.count}</td>
								...
								<td>${arr.address}</td>
								<td>
									<c:if test="${arr.status == 1}"><div class="text-success">通过</div></c:if>
									<c:if test="${arr.status == 2}"><div class="text-danger">拒绝</div></c:if>
								</td>
							</tr>
							</c:forEach>
						</c:if>
	...
	<script type="text/javascript">
		...
		function search() {
			...
			window.location.href = "business/delivery-history-list?userName="+user_name+"&positionName="+position_name+"&curPage="+curPage;
		}
